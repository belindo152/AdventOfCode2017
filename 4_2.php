...
lpyubo dylpfb iehwug decj ntidy cuygyg lalkb iutu oxgm imn'; 
	...
	$validPassPhrases = 0; // start at zero
	...
	foreach($passphrases as $passphrase) { // loop though passphrases
        ...
        $countAnagrams = 0; // start with 0 anagrams per passphrase
		for ($x = 0; $x < count($passphrasePart); $x++) { // loop though each of the wordgroups
			for ($y = 0; $y < count($passphrasePart); $y++) { // compare current wordgroups with each of the wordgroups
				if ($x <> $y) { // ignore comparing wordgroups with itself
					if ( count_chars($passphrasePart[$x],1) ==  count_chars($passphrasePart[$y],1)) { // check if chars are similair, if yes, add 1 to countAnagrams
						$countAnagrams++;
					}
				}
			}
	...
	echo $validPassPhrases; // and? amiright?
